...
export class PublicPlantsController {
  ...
  @ApiQuery({ name: 'limit', required: false, type: Number, description: 'Items per page (default: 12, max: 100)' })
  @ApiQuery({ name: 'search', required: false, type: String, description: 'Search by name, scientific name, common names' })
  @ApiQuery({ name: 'categoryId', required: false, type: String, format: 'uuid', description: 'Filter by category' })
  @ApiQuery({ name: 'tagIds', required: false, type: String, description: 'Comma-separated tag UUIDs' })
  @ApiQuery({ name: 'careDifficulty', required: false, enum: CareDifficultyEnum, description: 'Filter by care level' })
  @ApiQuery({ name: 'lightRequirement', required: false, enum: LightRequirementEnum, description: 'Filter by light requirement' })
  @ApiQuery({ name: 'wateringFrequency', required: false, enum: WateringFrequencyEnum, description: 'Filter by watering frequency' })
  @ApiQuery({ name: 'humidityLevel', required: false, enum: HumidityLevelEnum, description: 'Filter by humidity level' })
  @ApiQuery({ name: 'growthRate', required: false, enum: GrowthRateEnum, description: 'Filter by growth rate' })
  ...
  @ApiQuery({ name: 'inStockOnly', required: false, type: Boolean, description: 'Only show in-stock plants' })
  @ApiQuery({ name: 'sortBy', required: false, enum: ['name', 'price', 'difficulty', 'inventory', 'createdAt'], default: 'createdAt' })

 ...
 */
export const plantVariantTable = pgTable(
  'plant_variants',
  {
    ...
    /**
     * Indicates if this variant is the default variant for its plant.
     * Only one variant per plant should have isDefault = true.
     * Enforced by unique partial index: plant_variants_unique_default
     * Used for display priority and pre-selection in UI.
     */
  ...
  (t) => [
    // Partial unique index: enforce only one default variant per plant
    uniqueIndex('plant_variants_unique_default')
      .on(t.plantId)

...
import { shopFollowsTable } from './shop.follow.schema';
...
export const shopRelations = relations(shopTable, ({ one, many }) => ({
  ...
  whyChooseUs: many(shopWhyChooseUsTable),
  valuePoints: many(shopValuePointsTable),
  campaigns: many(shopCampaignsTable),
  articles: many(shopArticlesTable),
  follows: many(shopFollowsTable),
}));
